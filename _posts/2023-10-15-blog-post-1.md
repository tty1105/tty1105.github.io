...
```
There will be a progress bar in your outputs, and usually also some warnings raised by RADEX. The ```moves``` argument helps avoid local minimum, which can be removed is you are conducting a very simple fitting. The ```5000``` in ```sampler.run_mcmc``` means running 5000 steps of MCMC - it depends on the complexity of your fitting and the performance of your computer. <br>
...
Seems good! The points cluster at the best-fit values. 
