...
## SpectralRadex 
SpectralRadex需要从[源码](https://github.com/uclchem/SpectralRadex)安装，它需要的依赖比较奇怪，建议conda里新建一个环境来运行。我的包的版本是```python=3.11.5, numpy=1.26.4, pandas=2.3.3```。正常安装时会报一个关于```numpy.disutils```的错，这时需要```pip install "setuptools<60.0"```降级setuptools这个包，之后再安装SpectralRadex即可。后面安装别的包可能会把setuptools升级回来，但这样不会影响SpectralRadex的使用。
